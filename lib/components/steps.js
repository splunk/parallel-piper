...
function renderStep(n, state, { active, done }, content) {
  n = String(n);
  return (
    <section className={active ? "step active" : "step"} id={"step" + n}>
  ...
  return (
    <div className="steps">
      {renderStep(
        1,
        state,
        { active: !state.stateSelected, done: state.stateSelected },
        renderStateSelector
      )}
      {renderStep(
        2,
      ...
      {renderStep(3, state, { active: !!state.nameSubmitted }, (state) => (
        <div className="shake-it">SHAKE</div>
      ))}
    </div>
  );
}
